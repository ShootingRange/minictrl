...
impl<P: Player> Serialize for dyn Spectator<P> {
        ...
        S: Serializer,
    {
        ...
        let player_map: HashMap<String, String> = self
            .players()
            .iter()
            .map(|player| (player.steamID(), player.name().unwrap_or("".to_string())))
            .collect();
...
impl<P: Player> Serialize for dyn Team<P> {
    fn serialize<S>(&self, serializer: S) -> Result<<S as Serializer>::Ok, <S as Serializer>::Error>
    where
        S: Serializer,
    {
...
impl<P: Player, S: Spectator<P>, T: Team<P>> Serialize for dyn Match<P, S, T>
where
    P: Player,
    S: Spectator<P> + Serialize,
    T: Team<P> + Serialize,
{
        ...
        G: Serializer,
    {
        ...
        if let Some(favored_percentage_text) = self.favored_percentate_text() {
